...
function addStaticDHCPLease($mac, $ip, $hostname) {
	...
	try {
		...
		foreach($dhcp_static_leases as $lease) {
			if($lease["hwaddr"] === $mac)
			{
				throw new Exception("MAC地址 (".htmlspecialchars($mac).") 的静态地址分配已定义！<br>", 4);
			}
			if($ip !== "noip" && $lease["IP"] === $ip)
			{
				throw new Exception("IP地址 (".htmlspecialchars($ip).") 的静态地址分配已定义！<br>", 5);
			}
			if($lease["host"] === $hostname)
			{
				throw new Exception("主机名称 (".htmlspecialchars($hostname).") 的静态地址分配已定义！<br>", 6);
			}
		}
		...
		switch ($_POST["field"]) {
			...
			case "API":
				...
				foreach($domains as $domain)
				{
					...
					{
						$error .= "活跃域名/广告规则 ".htmlspecialchars($domain)." 无效（只使用域名）！<br>";
					}
					if(!$first)
					{
						$domainlist .= ",";
