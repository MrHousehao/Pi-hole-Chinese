    ...
    try {
        ...
        foreach ($names as $name) {
            ...
            if (!$stmt->bindValue(':name', $name, SQLITE3_TEXT)) {
                throw new Exception('绑定名称时：<strong>'.$db->lastErrorMsg().'</strong><br>已添加 '.$total.'个中的'.$added.' 个群组');
            }

            if (!$stmt->execute()) {
                throw new Exception('执行时：<strong>'.$db->lastErrorMsg().'</strong><br>已添加 '.$total.'个中的'.$added.'个群组');
            }
            ++$added;
        }
    ...
    try {
        ...
        foreach ($ips as $ip) {
            ...
            if (!$stmt->bindValue(':comment', $comment, SQLITE3_TEXT)) {
                throw new Exception('绑定描述时：<strong>'.$db->lastErrorMsg().'</strong><br>已添加'.$total.'个中的'.$added.'个客户端');
            }

            if (!$stmt->execute()) {
                throw new Exception('执行时：<strong>'.$db->lastErrorMsg().'</strong><br>已添加'.$total.'个中的'.$added.'个客户端');
            }
            ++$added;
        }
        ...
        foreach ($domains as $domain) {
            ...
            if ($_POST['action'] == 'replace_domain') {
                if (!$check_stmt->bindValue(':domain', $domain, SQLITE3_TEXT)) {
                    throw new Exception('检查绑定的域名时：<strong>'.$db->lastErrorMsg().'</strong><br>已添加'.$total.'个中的'.$added.'个域名');
                }

                $check_result = $check_stmt->execute();
                if (!$check_result) {
                    throw new Exception('执行检查时：<strong>'.$db->lastErrorMsg().'</strong><br>已添加'.$total.'个中的'.$added.'个域名');
                }
                ...
                if (!$delete_stmt->bindValue(':domain', $domain, SQLITE3_TEXT)) {
                    throw new Exception('绑定域名时：<strong>'.$db->lastErrorMsg().'</strong><br>已添加'.$total.'个中的'.$added.'个域名');
                }

                if (!$delete_stmt->execute()) {
                    throw new Exception('执行时：<strong>'.$db->lastErrorMsg().'</strong><br>已添加'.$total.'个中的'.$added.'个域名');
                }
            }
            ...
            if (!$insert_stmt->execute()) {
                throw new Exception('执行 INSERT OT IGNORE 时：<strong>'.$db->lastErrorMsg().'</strong><br>已添加'.$total.'个中的'.$added.'个域名');
            }
            ...
            if (!$update_stmt->execute()) {
                throw new Exception('执行UPDATE时：<strong>'.$db->lastErrorMsg().'</strong><br>已添加'.$total.'个中的'.$added.'个域名');
            }
            ++$added;
        }
    ...
    try {
        ...
        foreach ($addresses as $address) {
            ...
            if (preg_match('/[^a-zA-Z0-9:\\/?&%=~._()-;]/', $check_address) !== 0) {
                throw new Exception('<strong>无效的引力场URL'.htmlentities($address).'</strong><br>已添加 '.$total.'个中的'.$added.'个引力场');
            }

            if (!$stmt->bindValue(':address', $address, SQLITE3_TEXT)) {
                throw new Exception('绑定地址时：<strong>'.$db->lastErrorMsg().'</strong><br>已添加 '.$total.'个中的'.$added.'个引力场');
            }
            ...
            if (!$stmt->execute()) {
                if ($db->lastErrorCode() == 19) {
                    ...
                    throw new Exception('执行时：<strong>'.$db->lastErrorMsg().'</strong><br>已添加 '.$total.'个中的'.$added.'个引力场');
                }
            } else {
                ++$added;
        ...
        if ($ignored_list != '') {
            ...
            $msg = '<b>忽略重复的引力场：'.$ignored.'</b><br>'.$ignored_list;
            if ($added_list != '') {
                $msg .= '<br><b>已添加引力场：'.$added.'</b><br>'.$added_list;
            }
            $msg .= '<br><b>总计：'.$total.'个引力场已处理。</b>';
            JSON_warning($msg);
        } else {
            // All adlists added
            $msg = $added_list.'<br><b>总计：'.$total.'个引力场已处理。</b>';
            JSON_success($msg);
        }
    } catch (\Exception $ex) {
        JSON_error($ex->getMessage());
