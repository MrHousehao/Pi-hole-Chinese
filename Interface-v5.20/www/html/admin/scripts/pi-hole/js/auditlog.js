      ...
      if (Object.prototype.hasOwnProperty.call(data.top_queries, domain)) {
        ...
        domaintable.append(
          "<tr><td>" +
            ...
            "</td> <td>" +
            '<button type="button" class="btn btn-default btn-xs text-red btn-blacklist"><i class="fa fa-ban"></i> 添加到黑名单</button>' +
            '<button type="button" class="btn btn-default btn-xs text-orange btn-audit"><i class="fa fa-balance-scale"></i> 审查</button>' +
            "</td> </tr> "
        );
      }
    }
        ...
        if (input.length > 1) {
          ...
          adtable.append(
            "<tr><td>" +
              ...
              data.top_ads[domain] +
              "</td> <td>" +
              '<button type="button" class="btn btn-default btn-sm text-orange btn-audit"><i class="fa fa-balance-scale"></i> 审查</button>' +
              "</td> </tr> "
          );
        } else {
          ...
          adtable.append(
            "<tr><td>" +
              ...
              data.top_ads[domain] +
              "</td> <td>" +
              '<button type="button" class="btn btn-default btn-xs text-green btn-whitelist"><i class="fas fa-check"></i> 添加到白名单</button>' +
              '<button type="button" class="btn btn-default btn-xs text-orange btn-audit"><i class="fa fa-balance-scale"></i> 审查</button>' +
              "</td> </tr> "
          );
        }
