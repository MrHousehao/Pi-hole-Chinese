...
$('button[id="removedynamic"]').on("click", function () {
  ...
  utils.disableAll();
  utils.showAlert("info", "", "DHCP 静态地址分配删除中...", ipname);
  $.ajax({
    url: "api.php",
    method: "get",
    dataType: "json",
    ...
    success: function (response) {
      ...
      if (response.delete_lease.startsWith("OK")) {
        ...
        utils.showAlert("error", "删除 DHCP 静态地址分配" + ipname + "时出错", response);
      }
    },
    error: function (jqXHR, exception) {
      utils.enableAll();
      utils.showAlert("error", "删除 DHCP 静态地址分配" + ipname + "时出错", jqXHR.responseText);
