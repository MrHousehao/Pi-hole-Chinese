...
function check_cors() {
    ...
    if(isset($_SERVER['HTTP_HOST']) && !in_array($server_host, $AUTHORIZED_HOSTNAMES)) {
        log_and_die("主机检查失败：" . $server_host .' vs '. join(', ', $AUTHORIZED_HOSTNAMES));
    }
    ...
    if(isset($_SERVER['HTTP_ORIGIN'])) {
        ...
        if(!in_array($server_origin, $AUTHORIZED_HOSTNAMES)) {
            log_and_die("CORS失败：" . htmlspecialchars($server_origin) .' vs '. join(', ', $AUTHORIZED_HOSTNAMES));
        }
...
function list_verify($type) {
    ...
    elseif(isset($_POST['pw']))
    {
        ...
        {
            log_and_die("密码错误 - ".htmlspecialchars($type)."不允许列出".htmlspecialchars($_POST['domain'])."");
        }
    }
    else
    {
