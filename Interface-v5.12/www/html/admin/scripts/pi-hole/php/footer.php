        ...
        <div class="row row-centered text-center version-info">
            <div class="col-xs-12 col-sm-8 col-md-6">
                ...
                <ul class="list-unstyled">
                    <?php if($dockerTag) { ?> <li><strong>Docker 标签</strong> <?php echo $dockerTag; ?></li> <?php } ?>
                    <li><strong>Pi-hole 内核</strong> <?php echo $coreVersionStr; ?></li>
                    <li><strong>FTL</strong> <?php echo $ftlVersionStr; ?></li>
                    <li><strong>Web 界面</strong> <?php echo $webVersionStr; ?></li>
                </ul>
                <?php } else { ?>
                <ul class="list-inline">
                    ...
                    <li>
                        ...
                        <?php if ($core_update) { ?> &middot; <a class="lookatme" lookatme-text="可以更新！" href="<?php echo $coreReleasesUrl . "/latest"; ?>" rel="noopener" target="_blank">可以更新！</a><?php } ?>
                    </li>
                    <li>
                        <strong>FTL</strong>
                        <a href="<?php echo $ftlReleasesUrl . "/" . $FTL_current; ?>" rel="noopener" target="_blank"><?php echo $FTL_current; ?></a>
                        <?php if ($FTL_update) { ?> &middot; <a class="lookatme" lookatme-text="可以更新！" href="<?php echo $ftlReleasesUrl . "/latest"; ?>" rel="noopener" target="_blank">可以更新！</a><?php } ?>
                    </li>
                    <li>
                        <strong>Web 界面</strong>
                        <a href="<?php echo $webReleasesUrl . "/" . $web_current; ?>" rel="noopener" target="_blank"><?php echo $web_current; ?></a>
                        <?php if ($web_update) { ?> &middot; <a class="lookatme" lookatme-text="可以更新！" href="<?php echo $webReleasesUrl . "/latest"; ?>" rel="noopener" target="_blank">可以更新！</a><?php } ?>
                    </li>
                </ul>
                <?php if($core_update || $web_update || $FTL_update) { ?>
                <?php if($dockerTag) { ?>
                    <p>要安装更新，<a href="https://github.com/pi-hole/docker-pi-hole#upgrading--reconfiguring" rel="noopener" target="_blank">请使用新的生机映像替换此旧容器</a>。</p>
                <?php } else { ?>
                    <p>要安装更新，请在主机执行<code><a href="https://docs.pi-hole.net/main/update/" rel="noopener" target="_blank">pihole -up</a></code>命令。</p>
                <?php } ?>
                <?php } ?>
                <?php } ?>
            </div>
