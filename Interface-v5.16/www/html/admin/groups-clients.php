                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <p>客户端可通过IP地址（支持IPv4和IPv6）、
                           IP子网（CIDR表示法，如：<code>192.168.2.0/24</code>）、MAC地址（如：<code>12:34:56:78:9A:BC</code>),
                           ）、主机名（如：<code>localhost</code>）或通过所连接的接口（以冒号开头，如<code>:eth0</code>）的信息进行添加。</p>
                        <p>请注意，以IP地址（包括子网范围）配置的客户端识别优先级高于MAC地址、主机名或接口识别，主机名或接口识别的优先级在其之后。此外，MAC地址识别仅适用于距离Pi-hole较远的网络节点设备。
                        </p>
                    </div>
                </div>
            </div>
                ...
                <table id="clientsTable" class="table table-striped table-bordered" width="100%">
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th></th>
                            <th title="可接受的值包括：IP地址、 子网（CIDR表示法）、MAC地址 （AA:BB:CC:DD:EE:FF格式）或主机名。">客户端</th>
                            <th>描述</th>
                            <th>群组分配</th>
                            <th>&nbsp;</th>
                        </tr>
